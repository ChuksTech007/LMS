<nav class="bg-white shadow-lg z-50 sticky top-0" x-data="{ open: false, notificationOpen: false }">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="flex h-16 justify-between items-center">
            ...
            <div class="flex items-center space-x-3">
                <a href="{{ route('home') }}" class="flex items-center space-x-2 transition-transform hover:scale-105">
            ...
            <div class="hidden sm:flex items-center space-x-6 text-green-900 font-semibold">
                <a href="{{ route('courses.index') }}" class="hover:text-yellow-400 transition-colors duration-300">Courses</a>
                    ...
                    <div class="relative">
                        <button @click="notificationOpen = !notificationOpen" class="relative focus:outline-none p-2 rounded-full hover:bg-gray-100 transition-colors duration-300">
                            ...
                            @if($unreadNotifications->isNotEmpty())
                                <span class="absolute top-1 right-1 block h-2 w-2 rounded-full ring-2 ring-white bg-yellow-400"></span>
                            @endif
                        </button>
                        ...
                        <div x-show="notificationOpen" @click.away="notificationOpen = false"
                             ...
                             style="display: none;">
                            <div class="px-4 py-3 font-bold border-b border-gray-200 sticky top-0 bg-white flex items-center justify-between">
                                <span>Notifications</span>
                                @if($unreadNotifications->isNotEmpty())
                                    <span class="text-xs bg-yellow-400 text-green-900 px-2 py-0.5 rounded-full">{{ $unreadNotifications->count() }}</span>
                                @endif
                            </div>
                            @forelse ($unreadNotifications as $notification)
                                <a href="{{ route('notifications.read', $notification) }}"
                                   class="block px-4 py-3 text-sm hover:bg-green-50 transition-colors duration-200 border-b border-gray-50">
                                    <p class="font-medium leading-snug">{{ $notification->data['message'] }}</p>
                                    <p class="mt-1 text-xs text-gray-400">{{ $notification->created_at->diffForHumans() }}</p>
                                </a>
                            @empty
                                <p class="px-4 py-6 text-sm text-center text-gray-500">No new notifications.</p>
                            @endforelse
                        </div>
                    </div>
            ...
            <div class="sm:hidden">
                <button @click="open = !open" class="p-2 rounded-md text-green-900 hover:bg-green-100 focus:outline-none transition-colors duration-300">
                    ...
                    <svg :class="{ 'block': open, 'hidden': !open }" class="hidden h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div x-show="open"
         ...
         style="display: none;">
        <div class="space-y-1 px-4 py-4 font-semibold">
            <a href="{{ route('courses.index') }}" class="block px-3 py-2 rounded-md hover:bg-green-700 transition-colors duration-200">Browse Courses</a>
            @guest
            ...
            @auth
                ...
                @endif
                <a href="{{ route('profile.edit') }}" class="block px-3 py-2 rounded-md hover:bg-green-700 transition-colors duration-200">Profile</a>
                @if($unreadNotifications->isNotEmpty())
                    <div class="px-3 py-2 text-sm text-yellow-300">{{ $unreadNotifications->count() }} unread notification(s)</div>
                @endif
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
